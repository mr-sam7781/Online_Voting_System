...
/**
 * Servlet implementation class Candidate_ViewElections
 */
public class Candidate_ViewElections extends HttpServlet {
	Connection cn = null;
	Statement st = null; 
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(event.equals("Vote")) 
		{
			try
			{
				...
				String sql = "select *from votings where election_id='"+session.getAttribute("Election_Id")+"' && voter_id= '"+session.getAttribute("user_id")+"' && candidate_id = '"+candidate_id+"'";
				...
				{
					String sql1 = "insert into votings (election_id, voter_id, candidate_id) values ('"+session.getAttribute("Election_Id")+"','"+session.getAttribute("user_id")+"','"+candidate_id+"') ";
					...
					resp.setContentType("text/html");
					//out.println(" <script type=\"text/javascript\"> alert('Vote Submitted'); location= 'thankforyourvote.jsp'; </script>");     
				}
